...
可通过 `--validate-registry` 校验所有追溯到的源表是否已在注册表中，防止临时表未被完全穿透。
...
python3 run.py -H localhost -P 5432 -d tristan -u tristan \
    --proc p_gen_sale_report --schema public --format summary --validate-registry

python3 run.py -H localhost -P 5432 -d tristan -u tristan \
    --proc p_sale_analysis --schema public --format summary --validate-registry

python3 run.py -H localhost -P 5432 -d tristan -u tristan \
    --proc p_inventory_analysis --schema public --format summary --validate-registry

python3 run.py -H localhost -P 5432 -d tristan -u tristan \
    --proc p_cross_proc_demo --schema public --format summary --validate-registry --cross-proc
```
